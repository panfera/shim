...
void _write(EFI_FILE_PROTOCOL *file, CHAR16* file_buf)
{

	UINTN buf_size = RtStrSize(file_buf);
	file->Write(file, &buf_size, file_buf);

}

UINTN _findn (CONST CHAR16* str, CHAR16 symb){
...
efi_main (EFI_HANDLE passed_image_handle, EFI_SYSTEM_TABLE *passed_systab)
{
	...
	EFI_DEVICE_PATH_UTILITIES_PROTOCOL *DPUP = NULL;
	...
	CHAR16* file_name = L"info";

	CHAR16* s_dev_path = NULL;
	CHAR16* s_boot_path = NULL;
	...
	gBS->LocateProtocol(&gEfiDevicePathUtilitiesProtocolGuid, NULL, (void **)&DPUP);
	gBS->LocateProtocol(&gEfiDevicePathFromTextProtocolGuid, NULL, (void **)&DPFTP);

	gST->ConOut->ClearScreen(gST->ConOut);


	status = gBS->OpenProtocol(
		passed_image_handle, &gEfiLoadedImageProtocolGuid, (void **)&LIP, passed_image_handle, 
	...
	status = ST->BootServices->OpenProtocol(
		LIP->DeviceHandle, &gEfiDevicePathProtocolGuid, (void **)&dev_path,passed_image_handle,
		NULL, EFI_OPEN_PROTOCOL_GET_PROTOCOL);
	...
	boot_path = DPUP->AppendDeviceNode(dev_path, LIP->FilePath);

	s_dev_path = DPTTP->ConvertDevicePathToText(dev_path, FALSE, FALSE);

	s_boot_path = DPTTP->ConvertDevicePathToText(boot_path, FALSE, FALSE);


	ST->ConOut->OutputString(ST->ConOut, L"Dev_path: ");
	ST->ConOut->OutputString(ST->ConOut, s_dev_path);
	ST->ConOut->OutputString(ST->ConOut, L"\r\n");

	ST->ConOut->OutputString(ST->ConOut, L"Boot_path: ");
	ST->ConOut->OutputString(ST->ConOut, s_boot_path);
	ST->ConOut->OutputString(ST->ConOut, L"\r\n");

	CHAR16* s_dir_path;
	CONST CHAR16* s_f_path = DPTTP->ConvertDevicePathToText(LIP->FilePath, FALSE, FALSE);

	UINTN pos = _findn (s_f_path, '\\');
	pos+=1;
	s_dir_path = AllocatePool(pos);
	RtStrnCpy(s_dir_path, s_f_path, pos);
	s_dir_path[pos]='\0';


	CHAR16* s_file_path = AllocatePool(pos);
	RtStpCpy(s_file_path, s_dir_path);
	...
	EFI_FILE_PROTOCOL *file_handle = NULL;
            ...
            // получаем указатель на экземпляр протокола EFI_SIMPLE_FILE_SYSTEM_PROTOCOL для данного диска (файловой системы)
	status = gBS->HandleProtocol(LIP->DeviceHandle, &gEfiSimpleFileSystemProtocolGuid, (VOID **) &SFSP);
	...
	if (EFI_ERROR (status)) 
	{
		...
		return status;
            ...
            // создаём файл
	status = root->Open(root, &file_handle, s_file_path, EFI_FILE_MODE_CREATE | EFI_FILE_MODE_READ | EFI_FILE_MODE_WRITE, 0);
	...
	_write(file_handle, L"Dev_path: ");
	_write(file_handle, s_dev_path);
	_write(file_handle, L"\r\nFile_path: ");
	_write(file_handle, s_boot_path);
	_write(file_handle, L"\r\n");
